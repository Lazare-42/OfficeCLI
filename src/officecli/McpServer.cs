...
public static class McpServer
{
    ...
    /// client holds open for the life of the session. A handler that reads
    /// Console.In (batch with neither --commands nor --input, or --input -,
    /// falls through to Console.In.ReadToEnd) waits for an EOF that never
    /// arrives — and since this Invoke runs inline on the single reader loop
    /// in RunAsync, that hangs the WHOLE server permanently, not just the one
    /// call: every later request queues behind it unanswered until the process
    /// is restarted. It would also race the loop's own reader for protocol
    /// bytes. Null stdin turns that deadlock into an immediate empty-input
    /// parse error, which the agent can actually act on.
    ///
    /// None of the three redirects is restored afterwards, and that is the
    /// point. RunCliRaw is only ever reached in MCP mode, where the protocol
    /// stream is the private StreamWriter in RunAsync — Console.Out is never
    /// the transport, so pointing it back at real stdout buys nothing and costs
    /// a corruption channel: a command abandoned on the timeout below keeps
    /// running, and anything it prints later would land in the middle of the
    /// JSON-RPC stream. Leaving the redirects in place strands those writes
    /// harmlessly. Capturing a prevIn to restore would also force Console.In to
    /// materialize a reader over the transport pipe — the very thing being
    /// kept away from handlers.
    ///
    /// This redirect is the cheap half of the guard, and the fragile half: it
    /// only defuses handlers that read through Console.In. CommandBuilder
    /// .ReadBatchStdIn is the durable half, keying off McpServer.InMcpMode
    /// instead of the reader, so it still holds for a handler that opens the
    /// standard input handle directly and sails past Console.SetIn.
    /// </summary>
